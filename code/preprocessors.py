...
def tokenize_transcription_example(
    ...
    if audio_is_samples:
      ...
      if sample_rate != spectrogram_config.sample_rate:
        samples = librosa.resample(
            samples, orig_sr=sample_rate, target_sr=spectrogram_config.sample_rate)
    else:
  ...
  def process_record(input_record):
    ...
    if isinstance(input_record['audio'],tf.SparseTensor):
      audio = tf.sparse.to_dense(input_record['audio'])
    else:
      audio = input_record['audio']
